    ...
    private void getTransactions(boolean dev) {
        try {
            ...
            for (Transaction trans : transactions) {
                User user1;
                if (trans.getTransferCategory() == Transaction.Category.DEBIT) {
                    minus = "";
                    user1 = trans.getSender();

                } else {
                    minus = "-";
                    user1 = trans.getRecipient();
                }
                System.out.printf("To %s(id = %d) %s%d with id = %s\n",
                        user1.getName(), user1.getIdentifier(), minus,
