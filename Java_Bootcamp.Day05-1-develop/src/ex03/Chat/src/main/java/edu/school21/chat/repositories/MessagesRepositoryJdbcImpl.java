...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void update(Message message) {
        ...
        try (Connection con = dataSource.getConnection()) {
            PreparedStatement ps = con.prepareStatement("SELECT * FROM Chat.Message WHERE message_id = " + message.getMessageId());
            ...
            if (message.getTime() != null)
                time = Timestamp.valueOf(message.getTime());
            ps = con.prepareStatement("UPDATE chat.message SET author = ?, room = ?," + " text = ?, time = ? WHERE message_id = ?");
            ps.setLong(1, message.getAuthor().getUserId());
            ps.setLong(2, message.getRoom().getChatroomId());
            ps.setString(3, message.getText());
            ps.setTimestamp(4, time);
            ps.setLong(5, message.getMessageId());
            ps.executeUpdate();
            ps.close();
